...
const DevTools = memo(() => {
  ...
  const runApiTests = async () => {
    ...
    try {
      // Test 1: Authentification (si pas déjà connecté)
      if (!user) {
        addTestResult('Login Test', 'running', 'Testing login...');
        try {
          const result = await login('test@example.com', 'password123');
          addTestResult('Login Test', result.success ? 'success' : 'error', result);
        } catch (error: any) {
          addTestResult('Login Test', 'error', error?.message || 'Unknown error');
        }
      } else {
        addTestResult('Login Test', 'skipped', 'User already logged in');
      }
